...
//----------------------------------------------------------------------
extern "C" {
	rd_kafka_broker_s * rd_kafka_broker_any(
		rd_kafka_s *rk,
		int state,
		int(*filter) (rd_kafka_broker_s *rkb, void *opaque),
		void *opaque);
}
//----------------------------------------------------------------------
namespace qbus {
...
bool QbusProducerImp::Init(const QbusConfigLoader& config_loader_) {
 ...
 // QbusHelper::InitLog(logLevel, logPath);
  ...
  if (broker_list_.empty()) {
	  QBUS_ERROR(__FUNCTION__ << "broker_list_ is empty");
	  return false;
  }

  std::string topic_name = config_loader_.GetSdkConfig(RD_KAFKA_SDK_CONFIG_TOPIC_NAME, "");
  if (topic_name.empty()){
	  QBUS_ERROR(__FUNCTION__ << " topic name is empty");
	  return false;
  }

  is_init_ = QbusHelper::GetQbusBrokerList(config_loader_, &broker_list_)
	  && InitRdKafkaConfig(config_loader_) 
	  && InitRdKafkaHandle(topic_name);

  QBUS_INFO(__FUNCTION__ << " | broker list:" << broker_list_);
 ...
 //       RD_KAFKA_SDK_CONFIG_RECORD_MSG_DEFAULT);
	//if(0 == is_record_msg.compare("true")){
...
void QbusProducerImp::Uninit() {
  ...
  QBUS_INFO(__FUNCTION__ << " | Startting uninit...");

  if (NULL != rd_kafka_handle_ &&
	  NULL != rd_kafka_topic_) {
	  int current_poll_time = 0;
	  while (rd_kafka_outq_len(rd_kafka_handle_) > 0) {
		  QBUS_INFO(__FUNCTION__ << " | out queue is not empty, pull " << RD_KAFKA_POLL_TIMIE_OUT_MS << " ms...");
  ...
  QBUS_INFO(__FUNCTION__ << " | Finished uninit");
}

void QbusProducerImp::SetProduceCallback(ProduceCallback cb)
{
	produce_callback_ = cb;
}
...
bool QbusProducerImp::InternalProduce(const char* data,
  ...
  if (NULL == rd_kafka_handle_ ||
    ...
    if (RD_KAFKA_RESP_ERR_NO_ERROR != sync_send_err_) {
      QBUS_ERROR(__FUNCTION__ << " | Failed to sync produce"
          << " | topic: " << rd_kafka_topic_name(rd_kafka_topic_)
          << " | partition: " << -1 
          << " | msg error: " << rd_kafka_err2str(rd_kafka_resp_err_t(sync_send_err_)));
    } else {
      rt = true;
    }
  } else {
...
bool QbusProducerImp::InitRdKafkaHandle(const std::string& topic_name) {
  ...
  rd_kafka_handle_ = rd_kafka_new(RD_KAFKA_PRODUCER,
      ...
      sizeof(err_str));
  if (NULL == rd_kafka_handle_) {
    QBUS_ERROR(__FUNCTION__ << " | Failed to create new producer | QBUS_ERROR msg:" << err_str);
    ...
    if (NULL == rd_kafka_topic_) {
      QBUS_ERROR(__FUNCTION__ << " | Failed to rd_kafka_topic_new");
    } else {
      rt = true;
    }
  }
...
void QbusProducerImp::MsgDeliveredCallback(rd_kafka_s *rk,
            const rd_kafka_message_s *rkmessage, void *opaque) {
  QbusProducerImp* producer = static_cast<QbusProducerImp*>(opaque);
  if (NULL == producer) {
    QBUS_ERROR(__FUNCTION__ << " | Failed to static_cast from opaque to QbusProducerImp");
    return;
  }
  ...
  if (producer->is_sync_send_) {
    ...
    if (NULL == producer->rd_kafka_handle_ ||
        -1 == rd_kafka_produce(producer->rd_kafka_topic_,
          ...
          0,
          0)) {
      QBUS_ERROR(__FUNCTION__ << " | Failed to reproduce"
  ...
  if (rkmessage->err) {
    QBUS_ERROR(__FUNCTION__ << " | Failed to delivery message | err msg:" << rd_kafka_err2str(rkmessage->err)
	...
	if (producer->produce_callback_) {
		producer->produce_callback_(static_cast<char*>(rkmessage->payload), rkmessage->len, static_cast<char*>(rkmessage->key), rkmessage->key_len);
	}
    ...
    //}
	  ...
	  QBUS_DEBUG(__FUNCTION__ << " Successed to delivery message | bytes: " << rkmessage->len
...
int32_t QbusProducerImp::PartitionHashFunc(const rd_kafka_topic_s *rkt,
            const void *keydata,
            size_t keylen,
  ...
  if (keylen > 0 && NULL != keydata) {
	...
	//QBUS_DEBUG(__FUNCTION__ << " | KEY: " << std::string(key, keylen));
    ...
    if (1 != rd_kafka_topic_partition_available(rkt, hit_partition)) {
		QBUS_DEBUG(__FUNCTION__ << " | retry select parition | current invailed partiton: " << hit_partition);
      hit_partition = 0;
    }
  } else {
    ...
    while (true) {
      ...
      if (1 == rd_kafka_topic_partition_available(rkt, hit_partition)) {
        ...
        if (partition_set.empty()) {
			QBUS_DEBUG(__FUNCTION__ << " | failed to select parition | use RD_KAFKA_PARTITION_UA!");
          hit_partition = rd_kafka_msg_partitioner_random(rkt,
              keydata,
              keylen,
              partition_cnt,
  ...
  QBUS_DEBUG(__FUNCTION__ << " | hit_partition:" << hit_partition);

  return hit_partition;
}

bool QbusProducerImp::InitRdKafkaConfig(const QbusConfigLoader& config_loader_) {
	QBUS_INFO(__FUNCTION__ << " | Librdkafka version: " << rd_kafka_version_str() << " " << rd_kafka_version());
	...
	if (NULL != rd_kafka_conf_) {
		...
		if (NULL == rd_kafka_topic_conf_) {
			QBUS_ERROR(__FUNCTION__ << " | Failed to rd_kafka_topic_conf_new");
		}
		else {
			...
			if (!config_loader_.IsSetConfig(RD_KAFKA_TOPIC_MESSAGE_RETRIES, false)) {
				QBUS_DEBUG(__FUNCTION__ << " Set default global conf:" << RD_KAFKA_TOPIC_MESSAGE_RETRIES << " value:"<< RD_KAFKA_TOPIC_MESSAGE_RETRIES_VALUE);
			...
			if (!config_loader_.IsSetConfig(RD_KAFKA_CONFIG_TOPIC_METADATA_REFRESH_INTERVAL, false)) {
				QBUS_DEBUG(__FUNCTION__ << " Set default global conf:" << RD_KAFKA_CONFIG_TOPIC_METADATA_REFRESH_INTERVAL << " value:"<< RD_KAFKA_CONFIG_TOPIC_METADATA_REFRESH_INTERVAL_MS);
				QbusHelper::SetRdKafkaConfig(rd_kafka_conf_,
			...
			if (0 == sync_send.compare(RD_KAFKA_SDK_CONFIG_VALUE_SYNC_SEND)) {
			...
			//if (0 != minimize_producer_latency.compare(RD_KAFKA_SDK_CONFIG_PRODUCER_ENABLE_MINI_LATENCY_DEFAULT)) {
			//	//if (0 != strncasecmp(minimize_producer_latency.c_str(),
			//	//      RD_KAFKA_SDK_CONFIG_PRODUCER_ENABLE_MINI_LATENCY_DEFAULT,
			//	//      minimize_producer_latency.length())) {
			//	QBUS_DEBUG(__FUNCTION__ << " | enable minimize producer latency");
			//	QbusHelper::SetRdKafkaConfig(rd_kafka_conf_,
			//		RD_KAFKA_CONFIG_QUEUE_BUFFERING_MAX_MS,
			//		RD_KAFKA_SDK_MINIMIZE_PRODUCER_LATENCY_VALUE);
			//	QbusHelper::SetRdKafkaConfig(rd_kafka_conf_,
		...
		QBUS_ERROR(__FUNCTION__ << " | Failed to rd_kafka_conf_new");
	}
...
bool QbusProducer::init(const QbusConfigLoader& config_loader_)
{
	...
	if (i == kRmb->end()) {
	  ...
	  if (NULL != qbus_producer_imp_) {
		...
		if (rt) {
		  ...
		  QBUS_INFO(__FUNCTION__ << " | Procuder init is OK!");
		} else {
		  delete qbus_producer_imp_;
		  QBUS_ERROR(__FUNCTION__ << " | Failed to init");
		}
	  }
  ...
  #else*/
  if (NULL != qbus_producer_imp_) {
	  rt = qbus_producer_imp_->Init(config_loader_);
    if (rt) {
      QBUS_INFO(__FUNCTION__ << " | Procuder init is OK!");
    } else {
      QBUS_ERROR(__FUNCTION__ << " | Failed to init");
...
void QbusProducer::setProduceCallback(ProduceCallback cb)
{
	qbus_producer_imp_->SetProduceCallback(cb);
}
...
bool QbusProducer::produce(const char* data, size_t data_len,
  ...
  if (NULL != data &&
	  ...
	  QBUS_DEBUG(__FUNCTION__ << " | msg: " << std::string(data, data_len) << " | key: " << key);
    rt = qbus_producer_imp_->Produce(data, data_len, key);
  } else {
    QBUS_ERROR(__FUNCTION__ << " | Failed to produce | data is null: " << (NULL != data)
        << " | data len: " << data_len);
  }
